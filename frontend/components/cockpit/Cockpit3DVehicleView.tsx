...
import { Play, Square, RotateCcw, ArrowLeft, ArrowRight, Gauge, Sliders, ShieldCheck } from 'lucide-react';
...
export const Cockpit3DVehicleView: React.FC<Cockpit3DVehicleViewProps> = ({
  ...
  const propsRef = useRef({ heading, detectedObjects, nearbyV2VVehicles, threatLevel, speedKmH, manualMode });
  useEffect(() => {
    propsRef.current = { heading, detectedObjects, nearbyV2VVehicles, threatLevel, speedKmH, manualMode };
  }, [heading, detectedObjects, nearbyV2VVehicles, threatLevel, speedKmH, manualMode]);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const currentSteer = isManual ? steerAngleRef.current : 0;
          ...
          {manualMode && (
            <div className="flex items-center gap-1.5 shrink-0">
              <button
                ...
                className="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 border border-slate-200 font-bold text-slate-700 flex items-center gap-1 transition-all active:scale-95 cursor-pointer"
              ...
              <button
                ...
                className="px-3.5 py-1.5 rounded-lg bg-sky-600 hover:bg-sky-500 text-white font-extrabold flex items-center gap-1 shadow-xs transition-all active:scale-95 cursor-pointer"
              ...
              <button
                type="button"
                onClick={() => { setManualSpeed(0); setManualHeading(0); setSteerAngle(0); }}
